...
class ThemesPage(Gtk.Box):
    ...
    def refresh_themes(self) -> None:
        """Reconstrói a lista de temas. O container é trocado inteiro."""
        # Limpa container
        ...
        lb.connect(
            "row-activated",
            lambda box, row: self._apply_theme(row._theme_name, row._theme_kind),
        )
    ...
    def _make_theme_row(self, name: str, kind: str, active: str) -> Gtk.ListBoxRow:
        """
        Linha de tema como Gtk.ListBoxRow.
        ...
        row._theme_kind = kind   # type: ignore[attr-defined]
    ...
    def _apply_theme(self, name: str, kind: str) -> None:
        def task():
            ...
            if ok:
                GLib.idle_add(self._toast, f"✓  {name}")
